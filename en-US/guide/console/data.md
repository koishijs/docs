...
前后端的数据交互基本是通过 WebSocket 实现的。为了适应不同的场景，我们提供了多种数据交互的形式。为了适应不同的场景，我们提供了多种数据交互的形式。为了适应不同的场景，我们提供了多种数据交互的形式。
...
当你引入了 @koishijs/plugin-login 插件之后，你可以为你的页面访问和数据交互引入鉴权机制：
