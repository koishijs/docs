...
```tsx title=JSX
// @某某用户 我在叫你哟！
session.send(<><at id={userId}/>我在叫你哟！</>)
...
```ts title=API
// @某某用户 我在叫你哟！
session.send(h('at', { id: userId }) + '我在叫你哟！')
...
我们在 [这个页面](../../api/message/elements.md) 中列出了所有的标准元素，你可以稍后在这里查看每个元素的详细用法。
...
这些元素的用法与标准元素类似，但它们不在适配器中实现，而由 Koishi 直接处理。与之相对的，你只有在会话环境下才能使用它们 (非会话环境例如发送广播 `ctx.broadcast()` 等)。
