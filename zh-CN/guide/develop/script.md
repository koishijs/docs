...
::: tip
上面的 `-r esbuild-register` 来自 Koishi 模板项目的启动脚本，模板项目已经装好了这些依赖。如果你没有使用模板项目，需要先自行安装它们 (参见 [直接调用](../../manual/starter/direct.md))，或者改用 Node.js 原生的类型支持 (较新版本可以直接运行 `.ts` 文件，但 `enum`、`namespace` 和装饰器仍需真正的转换)。
:::
### 模块热替换 {#hmr}
