...
仅仅是在沙盒中对话是远远不够的。我们需要将机器人接入到真实的聊天平台中，才能让它真正地为我们服务。Koishi 使用适配器插件来支持各种聊天平台。其中，常用的适配器插件已经预装在了 Koishi 中，你可以在插件配置中的 adapter 分组中找到他们。
...
对于不同的平台，你需要做好相应的准备工作。以下是各个平台的官方适配器接入指南：
...
如果预装的插件中没有你想要的适配器，你可以在插件市场中搜索并安装更多的适配器插件。
